В данном репозитории проведена работа над решением задачи замены фона и генерации описания для карточек товаров.
...
Я написал генератор фона, для того, что бы придатьфону объем была реализована горизонтальная линия градиента,
положение которой зависит от самых нижних пикселей маски и создает эффект плоскости.
Такой подход отлично работает с предметами, которые находятся на плоскости, но в некоторых случаях это не совсем корректно.
...
- квантовать InternVL2 для оптимизации вычислений;
- заменить api GigaChat на локальную модельку;
